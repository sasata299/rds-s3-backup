...
class RdsS3Backup < Thor
  ...
  method_option :db_instance_class
  method_option :db_security_group_names, :desc => "Security group you want to attach to the db"
  ...
  def s3_dump
    ...
    backup_file_name     = "#{rds_server.id}-mysqldump-#{Date.today.strftime('%Y%m%d')}.sql.gz"
    ...
    rds.modify_db_instance(backup_server_id, true, 'DBSecurityGroups' => my_options[:db_security_group_names])
    sleep(120)
    ...
    begin
      ...
      cleanup(backup_server, backup_file_filepath)
      exit(1)
    end
  ...
  no_tasks do
    ...
    def cleanup(backup_server, backup_file_filepath)
